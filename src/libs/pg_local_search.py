...
class PgLocalSearch(BaseSearch):
    ...
    async def asearch(
        ...
        with st.chat_message("user", avatar="avatars/ms.svg"):
            with st.spinner(f'Local searching...'):
                ...
                with st.expander("Expand/Collapse"):
                    st.write(search_messages)

        with st.chat_message("assistant", avatar="avatars/openai.svg"):
    ...
    def search(
        ...
        st.write(f"GENERATE ANSWER: {start_time}. QUERY: {query}")
        try:
            search_prompt = self.system_prompt.format(
                context_data=context_text, response_type=self.response_type
            )
